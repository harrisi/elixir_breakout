defmodule Breakout.BallObject do
  ...
  def new(position, radius, velocity, sprite) do
    %__MODULE__{
      radius: radius,
      game_object: GameObject.new(position, Vec2.new(radius * 2, radius * 2), sprite, Vec3.new(1, 1, 1), velocity)
    }
  end
  ...
  def move(ball, dt, window_width) do
    unless ball.stuck do
      ...
      {new_x, new_y} = pos = ball.game_object.position
      |> Vec2.add(Vec2.scale(this_velocity, dt))

      {{new_velocity_x, new_velocity_y} = new_velocity, {new_position_x, new_position_y} = new_position} = cond do
        new_x <= 0 ->
        ...
        true -> {this_velocity, pos}
      end

      {new_velocity, new_position} = if new_position_y <= 0 do
