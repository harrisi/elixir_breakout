defmodule Breakout.Game do
  ...
  def init(_arg) do
    ...
    sprite_shader = Shader.init("priv/shaders/vertex.vs", "priv/shaders/fragment.fs")
    |> Shader.use_shader()
    ...
    state = put_in(state.resources.textures[:face], Texture2D.load("priv/textures/awesomeface.png", true))
    # |> IO.inspect(label: "face")
    # |> ResourceManager.put_texture(:face)

    state = put_in(state.resources.textures[:background], Texture2D.load("priv/textures/background.png", false))
    # |> ResourceManager.put_texture(:background)

    state = put_in(state.resources.textures[:block], Texture2D.load("priv/textures/block.png", false))
    # |> ResourceManager.put_texture(:block)

    state = put_in(state.resources.textures[:block_solid], Texture2D.load("priv/textures/block_solid.png", false))
    ...
    state = put_in(state.resources.textures[:paddle], Texture2D.load("priv/textures/paddle.png", true))
  ...
  def do_collisions(state) do
    ...
    {updated_bricks, ball} = Enum.map_reduce(cur_level.bricks, ball, fn box, ball_acc ->
      unless box.destroyed do
        {collided, dir, diff} = GameObject.check_collision(ball_acc, box)
        if collided do
          box = unless box.is_solid do
            %{box | destroyed: true}
          else
            box
          end
    ...
    ball = if not ball.stuck and paddle_collision do
      ...
      ball = %{ball | game_object: %{ball.game_object | velocity: {@initial_ball_velocity_x * percentage * strength, @initial_ball_velocity_y}}}
      ball = %{ball | game_object: %{ball.game_object | velocity: Vec2.normalize(ball.game_object.velocity) |> Vec2.scale(Vec2.length(old_vel))}}
    ...
    %Breakout.State{state |
      levels: put_elem(state.levels, state.level, level),
      ball: ball,
    }
  end
  ...
  defp resolve_collision(ball, dir, {diff_x, diff_y}) do
    ...
    case dir do
      :left ->
        ...
        %{ball | game_object: %{ball.game_object | position: {ball_pos_x + penetration, ball_pos_y}}}
      ...
      :up ->
        ...
        %{ball | game_object: %{ball.game_object | position: {ball_pos_x, ball_pos_y - penetration}}}
  ...
  def handle_cast({:key_down, key_code}, state) do
    state = %{state |
      ...
      level: if(key_code == ?N, do: rem(state.level + 1, tuple_size(state.levels)), else: state.level)
    }
  ...
  def handle_cast({:key_up, key_code}, state) do
    state = %{state |
      keys: MapSet.delete(state.keys, key_code),
    }
  ...
  def handle_info({:update, dt}, state) do
    ...
    state = if ball_y >= @screen_height do
      %{state |
  ...
  def handle_info({:process_input, dt}, state) do
    state = if state.game_state == :active do
      velocity = state.player.velocity * dt / 1_000# _000 #_000

      state =
        if MapSet.member?(state.keys, ?A) do
          {_, new_state} = get_and_update_in(state.player.position, fn {x, y} = orig ->
            if x >= 0 do
              {orig, {x - velocity, y}}
            else
              {orig, orig}
          ...
          new_ball = if player_x >= 0 and state.ball.stuck do
            {ball_x, ball_y} = state.ball.game_object.position
            b = BallObject.new(Vec2.new(ball_x - velocity, ball_y), state.ball.radius, state.ball.game_object.velocity, state.ball.game_object.sprite)
            %BallObject{
      ...
      state =
        if MapSet.member?(state.keys, ?D) do
          ...
          {player_x, _} = state.player.position
          new_ball = if player_x + 100 <= @screen_width and state.ball.stuck do
            {ball_x, ball_y} = state.ball.game_object.position
            b = BallObject.new(Vec2.new(ball_x + velocity, ball_y), state.ball.radius, state.ball.game_object.velocity, state.ball.game_object.sprite)
            %BallObject{
              game_object: b.game_object,
    ...
    state = if MapSet.member?(state.keys, ~c" " |> hd) do
      %Breakout.State{state |
        ball: %BallObject{
  ...
  def handle_info(:render, state) do
    :wx.batch(fn ->
      ...
      if state.game_state == :active do
        Sprite.draw(state, :background, Vec2.new(0, 0), Vec2.new(state.width, state.height), 0, Vec3.new(1, 1, 1))
  ...
  defp reset_level(state) do
    res = put_elem(state.levels, state.level, GameLevel.load("priv/levels/#{level_name(state.level)}", @screen_width, @screen_height / 2))
  ...
  defp reset_ball(state) do
    BallObject.reset(state.ball, Vec2.add(state.player.position, Vec2.new(@player_size_x / 2 - @ball_radius - 225, -@ball_radius * 2)), @initial_ball_velocity)
  end
end
